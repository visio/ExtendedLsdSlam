...
#include "myTestViewer/LSDSLAMViewerParamsConfig.h"
#include <qapplication.h>


#include "myTestViewer/keyframeGraphMsg.h"
#include "myTestViewer/keyframeMsg.h"
...
void dynConfCb(myTestViewer::LSDSLAMViewerParamsConfig &config, uint32_t level)
{
...
void frameCb(myTestViewer::keyframeMsgConstPtr msg)
{
...
void graphCb(myTestViewer::keyframeGraphMsgConstPtr msg)
{
	if(viewer != 0)
...
void rosThreadLoop( int argc, char** argv )
{
    ...
    ROS_INFO("myTestViewer started");

    dynamic_reconfigure::Server<myTestViewer::LSDSLAMViewerParamsConfig> srv;
	...
	rosbag::View view(bag, rosbag::TopicQuery(topics));
	 ...
	 BOOST_FOREACH(rosbag::MessageInstance const m, view)
	 {
		 ...
		 if(m.getTopic() == "/lsd_slam/liveframes" || m.getTopic() == "/lsd_slam/keyframes")
             frameCb(m.instantiate<myTestViewer::keyframeMsg>());


		 if(m.getTopic() == "/lsd_slam/graph")
             graphCb(m.instantiate<myTestViewer::keyframeGraphMsg>());
	 }
